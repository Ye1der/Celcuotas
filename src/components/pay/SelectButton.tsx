...
export function SelectButton ({label, selected, setSelected}: Props) {
  ...
  return (
    <button type="button" onClick={handleSelected} className={`${label === selected ? 'opacity-100 bg-smokyBlack bg-opacity-90' : 'opacity-50 bg-white'} group flex items-center justify-between border-2 border-smokyBlack border-opacity-90 hover:opacity-100 hover:bg-smokyBlack hover:bg-opacity-90 w-full px-4 py-2 rounded-2xl transition-all duration-300`}>
